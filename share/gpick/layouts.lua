...
layouts.add_buttons = function (container, buttons, percentage, button_style, text_style, spacing)
	...
	for i=0,buttons-1 do
		...
		button:add(layout:new_text("button_text", 0.25, 0.25, 0.5, 0.5, text_style[i % styles_text_n + 1], 'Button'));
		root:add(button);
	end;
	return container;
end;
...
gpick.layouts['std_layout_webpage_1'] = {
	...
	build = function (layout_system)
		...
		root:add(layouts.add_buttons(layout:new_fill("sidebar", 0, 0.15, 0.25, 0.8, styles['sidebar']), 5, 0.5, { styles['button'], styles['button'], styles['button_hover'] }, { styles['button_text'], styles['button_text'], styles['button_text_hover'] }, 0, 0));
